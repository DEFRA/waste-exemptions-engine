# frozen_string_literal: true

require "rails_helper"

module WasteExemptionsEngine
  RSpec.describe OperatorAddressLookupForm, type: :model do
    before { VCR.insert_cassette("postcode_valid") }
    after { VCR.eject_cassette }

    it_behaves_like "an address lookup form"

    it "validates the operator address using the AddressValidator class" do
      validators = build(:operator_address_lookup_form)._validators
      aggregate_failures do
        expect(validators[:operator_address].first.class)
          .to eq(WasteExemptionsEngine::AddressValidator)
      end
    end

    describe "#submit" do
      context "when the form is valid" do
        it "updates the transient registration with the selected address" do
          form = build(:operator_address_lookup_form)
          address_uprn = %w[340116 340117].sample
          valid_params = { operator_address: { uprn: address_uprn } }
          transient_registration = form.transient_registration

          aggregate_failures do
            expect(transient_registration.transient_addresses).to be_empty
            form.submit(valid_params)
            expect(transient_registration.transient_addresses.count).to eq(1)
            expect(transient_registration.transient_addresses.order(:id).first.uprn).to eq(address_uprn)
          end
        end
      end
    end

    it_behaves_like "a validated form", :operator_address_lookup_form do
      let(:valid_params) { { operator_address: { uprn: "340116" } } }
      let(:invalid_params) do
        [
          { operator_address: { uprn: "foo" } },
          { operator_address: { uprn: "" } }
        ]
      end
    end
  end
end
