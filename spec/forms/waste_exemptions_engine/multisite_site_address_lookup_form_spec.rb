...
module WasteExemptionsEngine
  RSpec.describe MultisiteSiteAddressLookupForm, type: :model do
    before do
      VCR.insert_cassette("postcode_valid")
    ...
    it "validates the site address using the AddressValidator class" do
      validators = build(:multisite_site_address_lookup_form)._validators
    ...
    describe "#submit" do
      context "when the form is valid" do
        it "updates the transient registration with the selected address" do
          form = build(:multisite_site_address_lookup_form)
    ...
    it_behaves_like "a validated form", :multisite_site_address_lookup_form do
