...
RSpec.shared_examples "an address lookup transition" do |next_state_if_not_skipping_to_manual:, address_type:, factory:|
  describe "#workflow_state" do
    ...
    subject(:registration) { create(factory, workflow_state: current_state) }

    context "when registration.skip_to_manual_address? is false" do
      ...
      it "is set to not skip to manual address" do
        expect(registration.send(:skip_to_manual_address?)).to be(false)
      end

      it "can only transition to #{next_state} or #{alt_state}" do
        permitted_states = Helpers::WorkflowStates.permitted_states(registration)
        expect(permitted_states).to contain_exactly(next_state, alt_state)
      end

      it "changes to #{next_state} after the 'next' event" do
        expect(registration).to transition_from(current_state).to(next_state).on_event(:next)
      end
    ...
    context "when registration.skip_to_manual_address? is true" do
      next_state = :"#{address_type}_address_manual_form"

      before { registration.address_finder_error = true }

      it "is set to skip to manual address" do
        expect(registration.send(:skip_to_manual_address?)).to be(true)
      end
