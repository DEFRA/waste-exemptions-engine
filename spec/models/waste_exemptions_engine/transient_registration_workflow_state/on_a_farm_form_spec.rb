  ...
  RSpec.describe TransientRegistration, type: :model do
    describe "#workflow_state" do
      ...
      subject(:transient_registration) do
        create(:transient_registration, workflow_state: current_state, addresses: [contact_address])
      end
      ...
      context "when a TransientRegistration's state is #{current_state}" do
        ...
        context "when transient_registration.contact_address_was_manually_entered? is true" do
          previous_state = :contact_address_manual_form

          before(:each) { transient_registration.contact_address.manual! }
          ...
          it "changes to #{previous_state} after the 'back' event" do
            expect(transient_registration.send(:contact_address_was_manually_entered?)).to eq(true)
            expect(transient_registration).to transition_from(current_state).to(previous_state).on_event(:back)
          end
        end

        context "when transient_registration.contact_address_was_manually_entered? is false" do
          ...
          it "changes to #{previous_state} after the 'back' event" do
            expect(transient_registration.send(:contact_address_was_manually_entered?)).to eq(false)
            expect(transient_registration).to transition_from(current_state).to(previous_state).on_event(:back)
          end
        end
      end
