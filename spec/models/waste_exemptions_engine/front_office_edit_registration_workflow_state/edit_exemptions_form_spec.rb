  ...
  RSpec.describe FrontOfficeEditRegistration do
    describe "#workflow_state" do
      ...
      context "when no changes have been made" do
        subject { create(:front_office_edit_registration, workflow_state: current_state) }

        it { expect(subject).to transition_from(current_state).to("front_office_edit_form").on_event(:next) }
      end
      ...
      context "when an exemption has been removed" do
        ...
        it { expect(subject).to transition_from(current_state).to("confirm_edit_exemptions_form").on_event(:next) }
      end
    end
  end
end
