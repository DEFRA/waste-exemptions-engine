...
module WasteExemptionsEngine
  RSpec.describe TransientRegistration do
    describe "#next_state!" do
      ...
      subject(:next_state) { transient_registration.next_state! }

      context "with no available next state" do
        before { transient_registration.workflow_state = "registration_complete_form" }

        it "does not change the state" do
          expect { next_state }.not_to change(transient_registration, :workflow_state)
        end

        it "does not add to workflow history" do
          expect { next_state }.not_to change(transient_registration, :workflow_history)
        end
      end

      context "with an invalid state" do
        before { transient_registration.workflow_state = "not_valid" }
      ...
      context "with a valid state" do
        ...
        it "changes the state" do
          expect { next_state }.to change(transient_registration, :workflow_state).to("business_type_form")
        end

        it "adds to workflow history" do
          expect { next_state }.to change { transient_registration.workflow_history.length }.from(0).to(1)
        end

        it "adds the previous state to workflow history" do
          expect { next_state }.to change(transient_registration, :workflow_history).to(["location_form"])
        end
      end
    end

    describe "#site_count" do
      let(:transient_registration) { create(:new_charged_registration, :with_all_addresses) }
      ...
      context "when is_multisite_registration is true" do
        ...
        it "returns the count of site addresses" do
          ...
          create(:transient_address, :site_address, transient_registration: transient_registration)
          expect(transient_registration.effective_site_count).to eq(3)
        end
      end
    ...
    describe "#previous_valid_state!" do
      ...
      subject(:previous_valid_state) { transient_registration.previous_valid_state! }

      context "with no workflow history" do
        before do
          transient_registration.workflow_history = []
          transient_registration.workflow_state = "location_form"
        end

        it "uses the default state" do
          expect { previous_valid_state }.to change(transient_registration, :workflow_state).to("start_form")
        end

        it "does not modify workflow history" do
          expect { previous_valid_state }.not_to change(transient_registration, :workflow_history)
        end
      end

      context "with partially invalid workflow history" do
        before { transient_registration.workflow_history = %w[another_form location_form not_valid] }

        it "skips the invalid state" do
          expect { previous_valid_state }.to change(transient_registration, :workflow_state).to("location_form")
        end

        it "deletes multiple items workflow history" do
          expect { previous_valid_state }.to change { transient_registration.workflow_history.length }.by(-2)
        end
      end
      ...
      context "with fully invalid workflow history" do
        before do
          transient_registration.workflow_state = "location_form"
          transient_registration.workflow_history = %w[no_start_form not_valid]
        end
        ...
        it "deletes all items from workflow history" do
          expect { previous_valid_state }.to change { transient_registration.workflow_history.length }.to(0)
        end
      end

      context "with valid workflow history" do
        before do
          transient_registration.workflow_history = %w[start_form location_form]
          transient_registration.workflow_state = "on_a_farm_form"
        end
        ...
        it "deletes from workflow history" do
          expect { previous_valid_state }.to change { transient_registration.workflow_history.length }.by(-1)
        end
      end

      context "when the current state is also in the workflow history" do
        before do
          transient_registration.workflow_history = %w[start_form location_form location_form]
          transient_registration.workflow_state = "location_form"
        end
    ...
    describe "#charged?" do
      context "when registration is not charged" do
        ...
        it { expect(transient_registration).not_to be_charged }
      end

      context "when registration is charged" do
        let(:transient_registration) { build(:new_charged_registration) }

        it { expect(transient_registration).to be_charged }
      end
    end
    ...
    describe "associations" do
      subject(:transient_registration) { create(:new_charged_registration, :complete) }
